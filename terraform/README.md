...
### IDDH production — apex and wildcard on the homeserver

The Hostinger website migration is complete. Verified in Cloudflare on
2026-09-09: `iddh.com.br`, `*.iddh.com.br`, `www.iddh.com.br` and
`membros.iddh.com.br` are proxied CNAMEs pointing at the homeserver tunnel.
The old apex A/AAAA records are gone; the import and deletion steps used for
that migration are no longer pending.

DNS (`local.public_tunnel_records`), tunnel routes (`local.tunnel_ingress`)
and the production Ingress (`gitops/production/iddh-members/ingress.yaml`)
cover the same hosts. One Phoenix deployment serves the institutional site
at the apex, the members app at `membros`, and event subdomains through the
wildcard. The app redirects `www` to the apex and `blog` to
`iddh.com.br/blog`. The existing explicit `www` and `membros` records remain
managed alongside the wildcard.

The separate staging environment remains at
`iddh-members-staging.prakash.com.br`, under the staging Cloudflare Access
policies. IDDH production uses `iddh.com.br` and its subdomains.

The website migration does not move email hosting. The existing apex MX/TXT
records remain in Cloudflare. Any mail-related hostname that needs another
target must retain its explicit record, which takes precedence over the
wildcard.
