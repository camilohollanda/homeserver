...
#   ./bootstrap/restic/configure.sh all         # both, in sequence
#
...
run_profile() {
  ...
  for job in $JOBS; do
    ...
    for path in "${paths[@]}"; do
      if ssh -o BatchMode=yes "$RESTIC_SSH" "sudo test -e '$path'" 2>/dev/null; then
        size=$(ssh -o BatchMode=yes "$RESTIC_SSH" "sudo du -sh '$path' 2>/dev/null | awk '{print \$1}'")
        printf "  ✓ %-7s %s\n" "$size" "$path"
      else
        printf "  ✗ MISSING  %s\n" "$path"
        missing+=("$path")
...
case "$profile" in
  services|k3s) run_profile "$profile" ;;
  all)
