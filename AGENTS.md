...
- **Public apps** (`*.werify.app`, `prakash.com.br`, `iddh.com.br`, `*.iddh.com.br`, `staging.miora.now`) → exposed via **Cloudflare Tunnel** from k3s-apps. **TLS terminates at the Cloudflare edge** — cloudflared forwards plain HTTP to `127.0.0.1:80` (ingress-nginx), and these Ingresses deliberately have **no `tls:` block**. cert-manager is not involved. Routes live in `terraform/cloudflare-tunnel.tf`; `*.werify.app`, `*.prakash.com.br` and `*.iddh.com.br` have wildcard routes. IDDH production already serves its apex and subdomains here. Miora has wildcard DNS (`*.miora.now`) configured, but only `staging.miora.now` has a tunnel route and Ingress for now. Unmatched tunnel hosts fall through to `http_status:404`.
